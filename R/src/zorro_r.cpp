...
SEXP C_zorro_gamma(SEXP rng_ptr, SEXP n_sexp, SEXP alpha_sexp) {
  zorro::Rng *rng = get_rng(rng_ptr);
  int n = INTEGER(n_sexp)[0];
  ...
  SEXP out = PROTECT(Rf_allocVector(REALSXP, n));
  rng->fill_gamma(REAL(out), static_cast<std::size_t>(n), alpha);
  UNPROTECT(1);
  return out;
}

SEXP C_zorro_student_t(SEXP rng_ptr, SEXP n_sexp, SEXP nu_sexp) {
  zorro::Rng *rng = get_rng(rng_ptr);
  int n = INTEGER(n_sexp)[0];
  ...
  SEXP out = PROTECT(Rf_allocVector(REALSXP, n));
  rng->fill_student_t(REAL(out), static_cast<std::size_t>(n), nu);
  UNPROTECT(1);
  return out;
}
...
static const R_CallMethodDef CallEntries[] = {
    ...
    {"C_zorro_gamma", (DL_FUNC)&C_zorro_gamma, 3},
    {"C_zorro_student_t", (DL_FUNC)&C_zorro_student_t, 3},
    {NULL, NULL, 0}};
